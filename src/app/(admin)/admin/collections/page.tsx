...
export default async function CollectionsPage() {
  ...
  const buckets = [
    { label: L.coll_b1, amount: a.feeAging.d0, bar: "bg-amber-200" },
    { label: L.coll_b2, amount: a.feeAging.d30, bar: "bg-amber-500" },
    { label: L.coll_b3, amount: a.feeAging.d60, bar: "bg-red-300" },
    { label: L.coll_b4, amount: a.feeAging.d90, bar: "bg-red-500" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label={L.adm_outstanding} value={formatCurrency(a.outstanding, cur)} tone={a.outstanding > 0 ? "red" : "green"} icon={<Banknote className="h-4 w-4" />} />
        {isSuper && (
          <StatCard label={`${L.coll_collected} · ${a.monthLabel}`} value={formatCurrency(a.collection.collected, cur)} tone="green" />
        )}
        {isSuper && (
          <StatCard label={L.coll_rate} value={a.collection.rate != null ? `${a.collection.rate}%` : "—"} tone={(a.collection.rate ?? 0) >= 80 ? "green" : "amber"} />
        )}
        <StatCard label={L.coll_overdue_accounts} value={rows.length} tone={rows.length ? "red" : "green"} icon={<Clock className="h-4 w-4" />} />
      </div>

      <Section title={L.coll_ageing}>
      ...
      <Section title={L.coll_overdue_accounts} description={L.ar_oldest_first} flush>
        {rows.length === 0 ? (
          <div className="p-5"><EmptyState icon={<CircleCheck className="h-5 w-5 text-green-500" />} message={L.msg_caught_up} hint={L.coll_empty_hint} /></div>
        ) : (
                ...
                <li key={i.id} className="flex flex-wrap items-center gap-3 px-5 py-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-slate-500">
                      {i.parent?.full_name ?? L.ar_no_parent} · <span className="font-mono">{i.invoice_no ?? "—"}</span>
                    </div>
                  </div>
                  <Badge tone={days >= 60 ? "red" : "yellow"}>{days} {L.coll_days_overdue}</Badge>
